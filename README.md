...
Bandwidth-saving delta updates using `.xcd` patch files.

### Process

```shell
1. Old package: hello-1.0.xcs (50MB)
2. Delta patch: hello-1.0-to-1.1.xcd (2MB)
...
2. **Read delta metadata**: Parse `delta.meta` from `.xcd` file
...
> [[!TIP]]
> Delta updates are ideal for frequent small updates. The `.xcd` format is generated by rLine during repository indexing.
