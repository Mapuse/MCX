...
impl SystemCommand {
    ...
    pub async fn rebuild(&self, config_path: &str) -> Result<()> {
        ...
        if !to_remove.is_empty() {
            let mut remover = RemoveCommand::new(self.root.to_string_lossy().into_owned(), Arc::clone(&self.db));
            if let Some(ref mgr) = self.plugin_mgr {
                remover = remover.with_plugin_mgr(Arc::clone(mgr));
            }
            remover.execute(&to_remove, &cgroup_mgr, &security_mon)
                .context("Atomic removal batch execution failed during state divergence alignment")? ;
        }

        if !to_install.is_empty() {
            let mut installer = InstallCommand::new(self.root.to_string_lossy().into_owned(), Arc::clone(&self.db))
