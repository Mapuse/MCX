...
impl SearchCommand {
    ...
    pub fn execute(&self, query: &str) -> Result<()> {
        ...
        let installed_names: std::collections::HashSet<String> = self.db.get_all_installed_packages()
            .unwrap_or_default()
            .into_iter()
            .map(|p| p.pkg_name)
            .collect();
        ...
        for pkg_name in available_packages {
            if pkg_name.pkg_name.to_lowercase().contains(&normalized_query) {
                match self.db.get_package_manifest(&pkg_name.pkg_name) {
                    Ok(meta) => {
                        let status_suffix = if installed_names.contains(&meta.pkg_name) { " [installed]" } else { "" };
