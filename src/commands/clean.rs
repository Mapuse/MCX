...
impl CleanCommand {
    ...
    pub fn execute(&self, clean_cache: bool, clean_history: bool) -> Result<()> {
        if !clean_cache && !clean_history {
            return Err(anyhow!(
                "No cleaning targets specified. Provide flags for cache or history."
            ));
        }

        if clean_cache && self.cache_dir.exists() {
            fs::remove_dir_all(&self.cache_dir).with_context(|| {
                format!(
                    "Failed to completely clear binary cache footprint at: {:?}",
                    self.cache_dir
                )
            })?;
        ...
        if clean_history && self.history_dir.exists() {
            fs::remove_dir_all(&self.history_dir).with_context(|| {
                format!(
                    "Failed to clear mutable historic log references at: {:?}",
                    self.history_dir
                )
            })?;
