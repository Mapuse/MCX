...
impl TuiEngine {
    pub fn load(cfg: &PythonConfig) -> Option<Self> {
        ...
        if !std_path.exists() {
            eprintln!("mcx: tui file not found: {}", path);
            return None;
        }
        ...
        match result {
            Ok(engine) => {
                eprintln!("mcx: loaded tui: {}", path);
                Some(engine)
            }
            Err(e) => {
                eprintln!("mcx: failed to load tui {}: {}", path, e);
                None
            }
        }
    }
...
impl TuiEngine {
    ...
    pub fn apply(entry: &TuiEntry) -> Result<String, String> {
        let _ = entry;
        eprintln!("mcx: tui apply not available in backward-compat mode");
