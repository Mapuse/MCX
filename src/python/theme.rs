...
impl ThemeEngine {
    pub fn load(cfg: &PythonConfig) -> Option<Self> {
        ...
        if !std_path.exists() {
            eprintln!("mcx: theme file not found: {}", path);
            return None;
        }
        ...
        match result {
            Ok(engine) => {
                eprintln!("mcx: loaded theme: {}", path);
                Some(engine)
            }
            Err(e) => {
                eprintln!("mcx: failed to load theme {}: {}", path, e);
                None
            }
        }
    }
    ...
    pub fn run(&self) -> bool {
        Python::with_gil(|py| {
            match self.module.call_method0(py, "run") {
                Ok(_) => true,
                Err(e) => {
                    eprintln!("mcx: python TUI exited: {}", e);
                    false
                }
            }
        })
...
impl ThemeEngine {
    ...
    pub fn apply(entry: &ThemeEntry) -> Result<String, String> {
        let _ = entry;
        eprintln!("mcx: theme apply not available in backward-compat mode");
