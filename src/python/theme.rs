...
fn theme_desc_candidates() -> Vec<std::path::PathBuf> {
    ...
    if let Ok(home) = std::env::var("HOME") {
        candidates.push(std::path::PathBuf::from(home).join(".config/mcx/t.desc"));
    }
    candidates.push(std::path::PathBuf::from("/etc/mcx/t.desc"));
    ...
    ONCE.call_once(|| {
        for path in theme_desc_candidates() {
                ...
                && let Ok(config) = toml::from_str::<ThemeDescConfig>(&content)
            {
                ...
                UserInterface::info(&format!("loaded themes from description File: {}", path.display()));
                return;
            }
        }
    });
